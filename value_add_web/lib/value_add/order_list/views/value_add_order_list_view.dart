...
class OrderItemView extends StatelessWidget {
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                buildRow(
                  'plan_validity_label'.tr,
                  '${getInterval()} ${'plan_days_label'.tr}',
                  0,
                ),
                buildRow(
                  'orders_order_id_label'.tr,
                  TextUtils.getStringWithOption(order.orderNumber),
                  0,
                ),
                buildRow(
                  'payment_method_menu_title'.tr,
                  TextUtils.getStringWithOption(order.paymentChannel),
                  0,
                ),

                buildRow(
                  'orders_order_date_label'.tr,
                  TextUtils.getStringWithOption(order.getPaidTime()),
                  0,
                ),
                buildRow(
                  'orders_deviceid_label'.tr,
                  TextUtils.getStringWithOption(order.deviceThirdPartId),
                  8,
                ),
                Row(
                  ...
                  children: [
                    Text.rich(
                      TextSpan(
                        ...
                        children: [
                          TextSpan(
                            text: TextUtils.getStringWithOption(
                              order.totalAmountDisplay,
                            ),
                            style: TextStyle(
                              fontSize: 20,
                              fontWeight: FontWeight.bold,
                              color: Color(0xFF0B6288),
                            ),
                          ),
                          TextSpan(
  ...
  String getPackageName() =>
      order.orderProductItems?.firstOrNull?.planNameSnapshot ?? "";
  String getInterval() {
    String billingType = order.billingType ?? "";
    return billingType == 'month'
        ? "30"
        : (billingType == 'year' ? "365" : "7");
  }
